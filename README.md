...
新規予約・予約変更・予約削除にはパスワード入力が必須です。新規予約時に入力したパスワードを、後で変更・削除するときにも使用します。


## 表示デザイン

予約カード内では、名前・使用時間・使用目的・備考をそれぞれ改行して表示します。

## v3 cache fix

この版では `script_v3.js` と `style_v3.css` を読み込むように変更しています。ブラウザやGitHub Pages CDNに古い `script.js` が残っていても、縦表示の新しいコードが確実に読み込まれます。

予約表示は、名前・使用時間・使用目的・備考をそれぞれ別行で表示します。
